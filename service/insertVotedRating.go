...
func InsertVotedRating(db *db.DataDB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
			...
			log.Printf("/voted Error decoding request body: %v", err)
			return
		}
		...
		if req.Who == nil {
			...
			log.Println("/voted User information is missing")
			return
		}
		...
		if isValidUser, id := db.SearchUser(*req.Who); !isValidUser {
			...
			log.Println("/voted Invalid user access")
			return
		} else {
			userId = id
		}
		...
		if err := db.InsertRating(req, userId); err != nil {
			...
			log.Printf("/voted Error inserting rating: %v", err)
			return
		}
		...
		log.Println("/voted success")
		if err := json.NewEncoder(w).Encode(response); err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			log.Printf("/voted Error encoding response: %v", err)
		}
	}
}
